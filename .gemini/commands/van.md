...
**No Gemini TaskMaster**: Bypass research coordination for practical development  

### **🔬 RESEARCH COORDINATION MODE** (Gemini TaskMaster Integration - RARE)
**Triggers**: System management, research project coordination, collective enhancement  
**Pattern**: Complex coordination through Gemini TaskMaster workflows  
**Full Orchestration**: Use enhanced Gemini TaskMaster agents (task-orchestrator → task-executor → task-checker)  
...
| **"execute tasks"** | **@task-orchestrator** | Coordinate existing Gemini TaskMaster tasks |
...
| **"migrate/convert agent to codex"** | **@convert-llm-gemini-to-codex** | Automated LLM migration |
...
| **🌟 Multi-Domain/Epic** | Decomposition and coordination needs | Always → `@task-orchestrator` with Gemini TaskMaster integration |
...
└── System Enhancement? → @task-orchestrator + Gemini TaskMaster
```
...
Task(subagent_type="component-implementation-agent", 
     prompt="Build React todo app with TypeScript - use Gemini for latest React patterns, implement TDD workflow")
```
...
Task(subagent_type="feature-implementation-agent",
     prompt="Implement authentication with security best practices - research latest patterns via Gemini, apply TDD methodology")
```
...
2. **Gemini TaskMaster Only When Needed**: Use @task-orchestrator for truly complex coordination (10% of requests)
3. **Research Integration**: Every agent incorporates Gemini research into their execution
...
3. **Research-Backed Routing**: Use Gemini patterns and Gemini TaskMaster data for informed routing
...
- **Research Integration**: [Gemini libraries / Gemini TaskMaster coordination]

## 🎯 Agent Execution Summary
**Agent**: @[agent-name]
**Task Delegated**: "[Exact task given to agent]"
**TDD Requirement**: [Yes/No + methodology]
**Research Context**: [Gemini libraries + research cache references]
