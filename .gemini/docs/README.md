# Gemini Code Sub-Agent Collective

## 📖 System Overview

Welcome to your Gemini Code Sub-Agent Collective installation! This system implements a research framework for reliable multi-agent coordination using hub-and-spoke architecture.

### What Just Happened?

The collective has been installed in your project with the following components:

#### 🧠 Behavioral Operating System (`GEMINI.md`)
...
#### 🧪 Testing Framework (`.gemini-collective/`)
...
### 1. Restart Gemini Code
**IMPORTANT**: Restart Gemini Code to activate the hook system and agents.
...
```bash
# Check status
npx gemini-code-collective status
...
### Monitoring Activity
- **Metrics**: Check `.gemini-collective/metrics/` for performance data
- **Logs**: Review `/tmp/collective-*.log` for detailed activity
- **Status**: Run `npx gemini-code-collective status` for health check
...
### Agent Configuration (`.gemini/agents/*.json`)
Each agent definition includes:
- **Capabilities**: What the agent can do
- **Tools**: Which Gemini Code tools they can access
- **Specialization**: Their primary focus area
- **Fallbacks**: Alternative agents if unavailable

### Testing Configuration (`.gemini-collective/`)
...
With the collective active, Gemini Code will behave differently:
...
1. **Restart Gemini Code** - Hooks need to be reloaded
2. **Check Status** - Run `npx gemini-code-collective status`
3. **Validate Installation** - Run `npx gemini-code-collective validate`
4. **Review Logs** - Check `/tmp/collective-*.log` files
5. **Repair Installation** - Run `npx gemini-code-collective repair`

### Getting Help
- **Status Command**: `npx gemini-code-collective status`
- **Validation**: `npx gemini-code-collective validate`  
- **Repair**: `npx gemini-code-collective repair`
- **Documentation**: Review the files in `.gemini/docs/`
...
# Fix problems
npx gemini-code-collective repair

# Remove collective
npx gemini-code-collective clean
```
