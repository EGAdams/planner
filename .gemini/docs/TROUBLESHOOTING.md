# Gemini Code Sub-Agent Collective - Troubleshooting Guide
...
**Solution**: 
1. Ensure you're using the latest version: `npx gemini-code-collective@latest`
2. Clear npm cache: `npm cache clean --force`
3. Try installing with `--force` flag: `npx gemini-code-collective --force`

### Permission Errors
**Problem**: EACCES errors when installing hooks or files.
**Solution**:
1. Run with appropriate permissions
2. Check directory ownership: `ls -la .gemini/`
3. Fix permissions: `chmod +x .gemini/hooks/*.sh`

### Hook Execution Failures
**Problem**: Hooks fail to execute or show permission denied.
**Solution**:
1. Make hooks executable: `chmod +x .gemini/hooks/*.sh`
2. Check shell compatibility (bash required)
3. Verify hook syntax: `bash -n .gemini/hooks/directive-enforcer.sh`
...
3. Restart Gemini Code to refresh agent registry

### Routing Failures
**Problem**: Requests don't route to expected agents.
**Solution**:
1. Check GEMINI.md routing patterns
...
**Problem**: Changes to `.gemini/settings.json` don't take effect.
**Solution**:
1. Restart Gemini Code completely
2. Check JSON syntax: `node -e "JSON.parse(require('fs').readFileSync('.gemini/settings.json'))"`
...
export DEBUG=gemini-collective:*

# Run with verbose output
npx gemini-code-collective --verbose --debug
```

## Getting Help

1. **Documentation**: Check `.gemini/docs/README.md` for system overview
2. **Test Results**: Run `npm test` in `.gemini-collective/` for system health
3. **Log Files**: Check `.gemini-collective/logs/` for detailed error logs
...
rm -rf .gemini/agents/*
rm -rf .gemini/hooks/*
rm -rf .gemini-collective/

# Reinstall
npx gemini-code-collective --force
```
