...
GEMINI_PROJECT_DIR=${GEMINI_PROJECT_DIR:-"/mnt/h/Active/taskmaster-agent-gemini-code"}
...
agent_tdd_checkpoint() {
    ...
    if [[ ! -d ".gemini-collective/node_modules" ]]; then
        log "Installing dependencies in .gemini-collective/ for testing..."
        (cd .gemini-collective && npm install > /dev/null 2>&1) || log "Failed to install dependencies"
    fi
    
    # Run vitest from .gemini-collective directory where dependencies are installed
    log "🧪 Running vitest validation for $agent_name..."
    
    timeout 60 bash -c "cd .gemini-collective && npx vitest run" > /tmp/agent-test-$agent_name.log 2>&1
    local exit_code=$?
...
execute_tdd_validation() {
    ...
    if [[ "$agent_name" == *"research"* ]]; then
        if echo "$agent_output" | grep -qi -E '(research|analysis|findings|documentation|Gemini|library)'; then
...
EOF
    ...
    if next_agent=$(detect_handoff "$AGENT_OUTPUT"); then
        ...
        # BREAKTHROUGH: Use Gemini Code's BLOCK mechanism to force continuation
        cat <<EOF
{
  "decision": "block",
