...
description: Specializes in Phase 3 hook integration including directive enforcement scripts, test-driven handoffs, and .gemini/settings.json configuration for behavioral system enforcement.
...
- Configure .gemini/settings.json with hook integration
...
- `.gemini/settings.json` hook configuration
...
```bash
# Get Task 3 details
mcp__task-master__get_task --id=3 --projectRoot=/mnt/h/Active/taskmaster-agent-gemini-code

# Update subtask status to in-progress
mcp__task-master__set_task_status --id=3.X --status=in-progress --projectRoot=/mnt/h/Active/taskmaster-agent-gemini-code

# Update task with progress
mcp__task-master__update_task --id=3.X --prompt="Hook implementation progress" --projectRoot=/mnt/h/Active/taskmaster-agent-gemini-code

# Mark subtask complete
mcp__task-master__set_task_status --id=3.X --status=done --projectRoot=/mnt/h/Active/taskmaster-agent-gemini-code
```
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": ".gemini/hooks/directive-enforcer.sh"
          }
        ]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command", 
            "command": ".gemini/hooks/quality-gate-validator.sh"
          }
        ]
      }
    ],
    "SubagentStop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": ".gemini/hooks/test-driven-handoff.sh"
          }
        ]
      }
    ]
...
3. **Integration**
   - Update .gemini/settings.json configuration
   - Test hook execution and validation
...
echo "IMPLEMENT DIRECTLY" | .gemini/hooks/directive-enforcer.sh

# Test handoff validation
HANDOFF_TOKEN="VALID_TEST_123" .gemini/hooks/test-driven-handoff.sh

# Test settings.json parsing
gemini-code /hooks validate
```
