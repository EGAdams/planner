...
- **ALWAYS deploy gemini-tdd-validation-agent BEFORE task closure** - Mandatory quality gate
...
CORRECT RESPONSE: "Deploying gemini-tdd-validation-agent to verify Task 1 before closure"
                  ↓
...
3. **COORDINATE TDD QUALITY GATES** - Deploy gemini-tdd-validation-agent for comprehensive TDD methodology validation
...
3. **Collective Agent Deployment**: You strategically deploy our specialized collective agents (@gemini-component-implementation-agent, @gemini-feature-implementation-agent, @gemini-infrastructure-implementation-agent, etc.) based on task requirements, ensuring each agent has Context7 research context and TDD methodology requirements.
...
2. When a task claims completion:
   - **FIRST: Deploy gemini-tdd-validation-agent to verify TDD compliance** - MANDATORY
...
- **MANDATORY: Deploy gemini-tdd-validation-agent BEFORE any task closure** - Not optional
...
✅ Task Tool Deployments Executed:
   - Task 1.x → Task(subagent_type="gemini-infrastructure-implementation-agent") EXECUTED
   - Task 2.x → Task(subagent_type="gemini-component-implementation-agent") EXECUTED
   - Task 3.x → Task(subagent_type="gemini-testing-implementation-agent") EXECUTED
   [LIST ALL ACTUAL TASK TOOL INVOCATIONS]

✅ Agent TDD Completion Reports Collected:
   - @gemini-infrastructure-implementation-agent: RED-GREEN-REFACTOR evidence provided
   - @gemini-component-implementation-agent: File system deliverables verified
...
Example endings:
- Use the gemini-infrastructure-implementation-agent subagent to implement Task ID 1.
- Use the gemini-component-implementation-agent subagent to implement Task ID 2.3.
- Use the gemini-feature-implementation-agent subagent to implement Task ID 5.
- Use the gemini-tdd-validation-agent subagent to validate TDD methodology compliance for completed tasks.
