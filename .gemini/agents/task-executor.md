...
2. SPAWN: Task(subagent_type="gemini-component-implementation-agent", prompt="Build X")
...
3. **Collective Agent Delegation**: 
   - **Route to specialized agents** based on task type:
     - UI/Frontend tasks → Task(subagent_type="gemini-component-implementation-agent")
     - Backend/API tasks → Task(subagent_type="gemini-feature-implementation-agent") 
     - Infrastructure/Build → Task(subagent_type="gemini-infrastructure-implementation-agent")
     - Testing/QA → Task(subagent_type="gemini-testing-implementation-agent")
...
9. **Route to gemini-task-checker** for final quality validation
...
```bash
# UI Component Task
Task(subagent_type="gemini-component-implementation-agent", 
     prompt="Build user login form component with Context7 React research, apply TDD methodology")

# Backend API Task  
Task(subagent_type="gemini-feature-implementation-agent",
