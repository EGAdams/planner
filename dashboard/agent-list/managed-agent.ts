...
export class ManagedAgent extends HTMLElement {
    ...
    private logs: string[] = [];
    private logPollInterval: number | null = null;
    ...
    attributeChangedCallback(name: string, oldValue: string, newValue: string) {
        ...
        if (name === 'name') this.agentName = newValue;
        if (name === 'description') this.agentDescription = newValue;
        if (name === 'running') {
    ...
    private async sendMessage(message: string) {
        if (!message.trim() || !this.agentId) return;
    ...
    `;
        ...
        const chatInput = this.shadowRoot.querySelector('.chat-input') as HTMLInputElement;
