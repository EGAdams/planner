...
- Reads `.gemini/handoff/NEXT_ACTION.json` for pending delegations
...
if [ -f .gemini/handoff/NEXT_ACTION.json ]; then
  echo "📋 PENDING HANDOFF FOUND:"
  cat .gemini/handoff/NEXT_ACTION.json | jq '.'
  echo ""
  echo "🎯 NEXT ACTION:"
  echo "Use the $(cat .gemini/handoff/NEXT_ACTION.json | jq -r '.target_agent') subagent to continue the workflow."
else
...
TARGET_AGENT=$(cat .gemini/handoff/NEXT_ACTION.json | jq -r '.target_agent' 2>/dev/null)
CONTEXT=$(cat .gemini/handoff/NEXT_ACTION.json | jq -r '.context' 2>/dev/null)
...
- **Session Restart**: After closing and reopening Gemini Code
...
```bash
if [ -f .gemini/handoff/NEXT_ACTION.json ]; then 
  TARGET=$(cat .gemini/handoff/NEXT_ACTION.json | jq -r '.target_agent')
