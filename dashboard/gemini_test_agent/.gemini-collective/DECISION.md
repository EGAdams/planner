...
1. **CHECK CONTEXT FILE**: Read `.gemini/handoff/NEXT_ACTION.json` if exists
...
**Handled by hooks (.gemini/hooks/test-driven-handoff.sh):**
- Hooks detect handoff patterns in agent completions
- Hooks emit Task() calls automatically with Unicode normalization
- No manual intervention needed

## ROUTING DECISIONS
**For /gemini commands**: User explicitly requested collective mode - load full behavioral context from GEMINI.md
**For normal questions**: Use standard Gemini behavior - ignore collective behavioral rules
...
- **/gemini command used**: Load collective behavioral rules and routing matrices
- **Normal conversation**: Standard Gemini behavior + auto-delegation capability
