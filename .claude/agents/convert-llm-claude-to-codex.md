---
name: convert-llm-claude-to-codex
description: Automates migration of Claude-based agents to GPT-5-Codex LLM with comprehensive validation and rollback capabilities. Uses TDD methodology to ensure safe, tested migrations.
...
I automate the complete migration process for converting any Claude-based agent to use GPT-5-Codex as the underlying language model, with full TDD validation and comprehensive testing.
...
1. **Validate Request**
   ...
   test -f .claude/agents/{agent-name}.md
   ```

2. **Create Backup**
   ```bash
   # Timestamped backup
   cp .claude/agents/{agent-name}.md .claude/agents/backups/{agent-name}_backup_$(date +%Y%m%d_%H%M%S).md
   ```

3. **Read Current Agent**
   ```javascript
   Read(file_path=".claude/agents/{agent-name}.md")
...
4. **Create Migration Tests**
   ...
   // Test 4: Integration notes present
   // Test 5: No content corruption
   ```
...
7. **Update Agent File**
   ```javascript
   Edit(file_path=".claude/agents/{agent-name}.md",
        old_string="---\n\n## [First Section]",
        new_string=`---\n\n${llmConfig}\n\n${bridgeConfig}\n\n## [First Section]`)
   ```
...
10. **Update Documentation**
    ...
    Edit(file_path=".claude-collective/agents.md")
    // Add Codex badge to agent entry

    // Update van routing if needed
    Edit(file_path=".claude/commands/van.md")
...
describe('Agent Codex Migration: {agent-name}', () => {
  const agentContent = readFileSync('.claude/agents/{agent-name}.md', 'utf-8');
...
1. **Identify Backup**
   ```bash
   ls -lt .claude/agents/backups/{agent-name}_backup_* | head -1
   ```

2. **Restore Original**
   ```bash
   cp .claude/agents/backups/{agent-name}_backup_TIMESTAMP.md .claude/agents/{agent-name}.md
   ```
...
### Agent Migrated
**Name**: {agent-name}
**Original File**: .claude/agents/{agent-name}.md
**Backup Location**: .claude/agents/backups/{agent-name}_backup_{timestamp}.md
**Migration Date**: {date}
...
- Working Directory: /home/adamsl/codex_claude_communication
...
- Van routing patterns verified
- Migration example added to documentation

### Rollback Available
To rollback this migration:
```bash
cp {backup_location} .claude/agents/{agent-name}.md
```
