...
CODEX_PROJECT_DIR=${CODEX_PROJECT_DIR:-$(cd "$SCRIPT_DIR/../.." && pwd)}
...
agent_tdd_checkpoint() {
    ...
    if [[ ! -d ".codex-collective/node_modules" ]]; then
        log "Installing dependencies in .codex-collective/ for testing..."
        (cd .codex-collective && npm install > /dev/null 2>&1) || log "Failed to install dependencies"
    fi
    
    # Run vitest from .codex-collective directory where dependencies are installed
    log "🧪 Running vitest validation for $agent_name..."
    
    timeout 60 bash -c "cd .codex-collective && npx vitest run" > /tmp/agent-test-$agent_name.log 2>&1
    local exit_code=$?
...
EOF
    ...
    if next_agent=$(detect_handoff "$AGENT_OUTPUT"); then
        ...
        # BREAKTHROUGH: Use OpenAI Codex's BLOCK mechanism to force continuation
        cat <<EOF
{
  "decision": "block",
