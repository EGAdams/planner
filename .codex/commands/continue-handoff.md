...
- Reads `.codex/handoff/NEXT_ACTION.json` for pending delegations
...
if [ -f .codex/handoff/NEXT_ACTION.json ]; then
  echo "📋 PENDING HANDOFF FOUND:"
  cat .codex/handoff/NEXT_ACTION.json | jq '.'
  echo ""
  echo "🎯 NEXT ACTION:"
  echo "Use the $(cat .codex/handoff/NEXT_ACTION.json | jq -r '.target_agent') subagent to continue the workflow."
else
...
TARGET_AGENT=$(cat .codex/handoff/NEXT_ACTION.json | jq -r '.target_agent' 2>/dev/null)
CONTEXT=$(cat .codex/handoff/NEXT_ACTION.json | jq -r '.context' 2>/dev/null)
...
- **Session Restart**: After closing and reopening OpenAI Codex
...
```bash
if [ -f .codex/handoff/NEXT_ACTION.json ]; then 
  TARGET=$(cat .codex/handoff/NEXT_ACTION.json | jq -r '.target_agent')
