...
description: Specializes in Phase 3 hook integration including directive enforcement scripts, test-driven handoffs, and .codex/settings.json configuration for behavioral system enforcement.
...
- Configure .codex/settings.json with hook integration
...
```bash
# Get Task 3 details
mcp__task-master__get_task --id=3 --projectRoot=/home/adamsl/codex_tdd_orchestrator

# Update subtask status to in-progress
mcp__task-master__set_task_status --id=3.X --status=in-progress --projectRoot=/home/adamsl/codex_tdd_orchestrator

# Update task with progress
mcp__task-master__update_task --id=3.X --prompt="Hook implementation progress" --projectRoot=/home/adamsl/codex_tdd_orchestrator

# Mark subtask complete
mcp__task-master__set_task_status --id=3.X --status=done --projectRoot=/home/adamsl/codex_tdd_orchestrator
```
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": ".codex/hooks/directive-enforcer.sh"
          }
        ]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command", 
            "command": ".codex/hooks/quality-gate-validator.sh"
          }
        ]
      }
    ],
    "SubagentStop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": ".codex/hooks/test-driven-handoff.sh"
          }
        ]
      }
    ]
...
3. **Integration**
   - Update .codex/settings.json configuration
   - Test hook execution and validation
...
echo "IMPLEMENT DIRECTLY" | .codex/hooks/directive-enforcer.sh

# Test handoff validation
HANDOFF_TOKEN="VALID_TEST_123" .codex/hooks/test-driven-handoff.sh

# Test settings.json parsing
codex-code /hooks validate
```
