# OpenAI Codex Sub-Agent Collective - Troubleshooting Guide
...
**Solution**: 
1. Ensure you're using the latest version: `npx codex-code-collective@latest`
2. Clear npm cache: `npm cache clean --force`
3. Try installing with `--force` flag: `npx codex-code-collective --force`

### Permission Errors
**Problem**: EACCES errors when installing hooks or files.
**Solution**:
1. Run with appropriate permissions
2. Check directory ownership: `ls -la .codex/`
3. Fix permissions: `chmod +x .codex/hooks/*.sh`

### Hook Execution Failures
**Problem**: Hooks fail to execute or show permission denied.
**Solution**:
1. Make hooks executable: `chmod +x .codex/hooks/*.sh`
2. Check shell compatibility (bash required)
3. Verify hook syntax: `bash -n .codex/hooks/directive-enforcer.sh`
...
3. Restart OpenAI Codex to refresh agent registry
...
**Problem**: Changes to `.codex/settings.json` don't take effect.
**Solution**:
1. Restart OpenAI Codex completely
2. Check JSON syntax: `node -e "JSON.parse(require('fs').readFileSync('.codex/settings.json'))"`
...
export DEBUG=codex-collective:*

# Run with verbose output
npx codex-code-collective --verbose --debug
```

## Getting Help

1. **Documentation**: Check `.codex/docs/README.md` for system overview
2. **Test Results**: Run `npm test` in `.codex-collective/` for system health
3. **Log Files**: Check `.codex-collective/logs/` for detailed error logs
...
rm -rf .codex/agents/*
rm -rf .codex/hooks/*
rm -rf .codex-collective/

# Reinstall
npx codex-code-collective --force
```
