...
# Pull these from your existing May builder module.
# This assumes create_may_spreadsheet.py contains:
# - SpreadsheetRow dataclass
# - gather_rows() that returns List[SpreadsheetRow]
# - DEFAULT_ORG_ID, settings, etc.
from create_may_spreadsheet import (
    DEFAULT_ORG_ID,
    SpreadsheetRow,
    gather_rows,
    settings,
...
OUTPUT_FILE = OUTPUT_DIR / "may_2025_spreadsheet.xlsx"
...
def write_spreadsheet(rows: List[SpreadsheetRow]) -> Path:
    ...
    ws = workbook.add_worksheet("May 2025")
    ...
    # Title + metadata
    # ----------------------------
    ws.write(0, 0, "May 2025 Financial Activity (MySQL Source)", title_fmt)
