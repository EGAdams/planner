#!/usr/bin/env python3
"""Build a May 2025 spreadsheet directly from the MySQL data."""
...
OUTPUT_FILE = OUTPUT_DIR / "may_2025_spreadsheet.csv"
...
def fetch_transactions(cnx) -> List[SpreadsheetRow]:
    ...
    with cnx.cursor(dictionary=True) as cursor:
        cursor.execute(query, (DEFAULT_ORG_ID, MAY_START, MAY_END))
...
def gather_rows() -> List[SpreadsheetRow]:
    ...
    if not rows:
        raise RuntimeError("No May 2025 data found in transactions or expenses.")
...
def write_spreadsheet(rows: List[SpreadsheetRow]) -> Path:
    ...
    with OUTPUT_FILE.open("w", newline="", encoding="utf-8") as csv_file:
        ...
        writer.writerow(["May 2025 Financial Activity (MySQL Source)"])
        writer.writerow(
            [
...
def main() -> None:
    ...
    print(f"✓ May spreadsheet created: {spreadsheet_path}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"✗ Failed to build May spreadsheet: {exc}")
        raise
