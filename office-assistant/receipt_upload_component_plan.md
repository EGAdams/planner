...
Design and implement a complete receipt scanning and parsing system for our nonprofit finance application. The system should:
...
- **Database:** MySQL 8.0.43 (running on localhost:3306, database name: `nonprofit_finance`) Credentials in envirionment variables.
...
**Alternative Fallback Providers** (implement as pluggable engines):
- **OpenAI gpt-4o-mini:** ~$0.10 per 100 receipts, excellent reasoning
- **OpenAI gpt-5-nano:** (if available when implemented) cheaper alternative to gpt-4o-mini
- **Google Document AI:** Specialized receipt processor, free tier covers 100/month
- **Azure Document Intelligence:** Same as Google, free tier covers 100/month

**Architecture Requirement:**
Design `receipt_parser.py` with a pluggable `ReceiptEngine` interface so you can:
- Start with `GeminiReceiptEngine` (default)
- Add `OpenAIReceiptEngine` later if needed
- Add `DocumentAIEngine` for high-volume scenarios
- Easily switch engines via configuration
